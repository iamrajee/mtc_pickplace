...
moveit_msgs::CollisionObject createObject() {
	...
	std::size_t errors = 0;
	errors += !rosparam_shortcuts::get(LOGNAME, pnh, "object_name", object_name);
	errors += !rosparam_shortcuts::get(LOGNAME, pnh, "object_reference_frame", object_reference_frame);
	errors += !rosparam_shortcuts::get(LOGNAME, pnh, "object_dimensions", object_dimensions);
	errors += !rosparam_shortcuts::get(LOGNAME, pnh, "object_pose", pose);
	...
	pose.position.z +=object_dimensions[1]; //??
	object.primitive_poses.push_back(pose);
	return object;
}
